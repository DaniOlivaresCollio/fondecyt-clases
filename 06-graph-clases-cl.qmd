...
# Gráfico vertical agrupado
grafico <- ggplot(class_pct_sex, aes(x = wrightclass_v15_emp_reduced, y = pct, fill = sexo)) +
  geom_col(position = position_dodge(width = 0.7), width = 0.65) +
  geom_text(
    aes(label = paste0(round(pct, 1), "%")),
    position = position_dodge(width = 0.7),
...
grafico
ggsave("output/graph-clases/wrightclass_v15_sexo_vert.png", plot = grafico, width = 10, height = 8, dpi = 300)
```

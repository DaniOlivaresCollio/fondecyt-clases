...
Capítulo para el procesamiento de la Data Conclat Chile, actualizada para el 26/03/2026.
...
data <- data %>% 
  set_variable_labels(ciuo08_ine = "ISCO 08")
...
data <- data %>%
  mutate(
    formal_position_auth = factor(
      if_else(
        as.numeric(relation_mop) %in% c(1, 2), NA_real_, as.numeric(a15)
      ),
      levels = 1:4,
      labels = c(
        "Senior manager or executive",
        "Middle manager (e.g., sector, office or branch head)",
        "Supervisor or foreperson",
        "Regular employee or worker (no supervisory role)"
      ), 
      ordered = TRUE
    )
  ) %>%
...
data <- data %>% 
  ...
  set_variable_labels(inicio_empresa = "Business registration 2 (1 = yes)") %>% 

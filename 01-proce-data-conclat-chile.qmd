...
Procesamiento de Data Conclat Chile actualizada para el 25/03/2026.
...
data <- readRDS("input/data/data-conclat-chile-ES.rds")
```
...
data <- data %>%
  ...
  set_variable_labels(
    ...
    relation_mop = "Relation with Means of Production")
...
data <- data %>% 
  ...
  set_variable_labels(a06_boleta = "Issues receipts/invoices (1 = yes)") %>% 
...
data <- data %>% 
  ...
  set_variable_labels(inicio_empresa = "Incio de actividades 2 (1 = yes)") %>% 
...
### wrightclass2_reduced2 (08 es igual al anterior)
...
### wrightclass1_reduced3_v2

```{r}
data <- data %>%
  mutate(
    wrightclass1_reduced3_v2 = case_when(
      as.integer(owners) %in% c(2, 3) ~ 8,                                                                    # 8. Small employers   
      as.integer(owners) == 1 & a06_boleta == 1 ~ 7,                                                          # 7. Petty bourgeoisie  
      
      as.integer(organization_assets) == 1 & as.integer(skills) == 1 ~ 6,                                     # 6. Expert managers   
      as.integer(organization_assets) == 3 & as.integer(skills) == 1 ~ 5,                                     # 5. Experts  
      
      as.integer(organization_assets) %in% c(1, 2) & as.integer(skills) %in% c(2, 3) ~ 4,                     # 4. Skilled y Unskilled Supervisors
      as.integer(organization_assets) == 2 & as.integer(skills) == 1 ~ 4,                                     # 4. Skilled y Unskilled Supervisors

      as.integer(organization_assets) == 3 & as.integer(skills) %in% c(2, 3) & a18 == 1 ~ 1,                  # 1. Formal workers
      as.integer(organization_assets) == 3 & as.integer(skills) %in% c(2, 3) & a18 %in% c(2, 3, 4, -999) ~ 2, # 2. Informal workers
      as.integer(owners) == 1 & a06_boleta == 0 ~ 3                                                           # 3. Informal self-employed
    ),
    wrightclass1_reduced3_v2 = factor(
      wrightclass1_reduced3_v2,
      levels = 1:8,
      labels = c(
        "1. Formal workers",
        "2. Informal workers",
        "3. Informal self-employed",
        "4. Skilled & Unskilled Supervisors",
        "5. Experts",
        "6. Expert managers",
        "7. Petty bourgeoisie",
        "8. Employers"
  ...
  set_variable_labels(wrightclass1_reduced3_v2 = "Wright Class Typology Reduced (9v2)")

frq(data$wrightclass1_reduced3_v2)
...
# Tabla de frecuencias ponderadas
wright_freq <- data_svy %>%
  group_by(wrightclass1_reduced3_v2) %>%
  summarise(
    freq = survey_mean(vartype = NULL, na.rm = TRUE),
    n_unweighted = unweighted(n())
  ) %>%
...
data <- data %>%
  ...
  set_variable_labels(wrightclass2_reduced4 = "Wright Class Typology Reduced (12)")
...
data <- data %>%
  mutate(
    wrightclass1_reduced5 = case_when(
      ...
      as.integer(owners) == 1 & a06_boleta == 1 & inicio_activ %in% c(1) ~ 3,                                 # 3. Petty bourgeoisie  
      ...
      as.integer(organization_assets) == 3 & as.integer(skills) %in% c(2, 3) & a18_contrato == 1 ~ 7,         # 7. Formal workers
      
      as.integer(organization_assets) == 3 & as.integer(skills) %in% c(2, 3) & a18_contrato == 0 ~ 8,         # 8. Informal workers
      
      as.integer(owners) == 1 & a06_boleta == 0 ~ 9,                                                          # 9. Informal self-employed
      as.integer(owners) == 1 & a06_boleta == 1 & inicio_activ == 0 ~ 9                                       # 9. Informal self-employed
    ),
    wrightclass1_reduced5 = factor(
      wrightclass1_reduced5,
      levels = 1:9,
      labels = c(
        "1. Employers",
  ...
  set_variable_labels(wrightclass1_reduced5 = "Wright Class Typology Reduced (13)")

frq(data$wrightclass1_reduced5)
...
# Tabla de frecuencias ponderadas
wright_freq <- data_svy %>%
  group_by(wrightclass1_reduced5) %>%
  summarise(
    freq = survey_mean(vartype = NULL, na.rm = TRUE),
    n_unweighted = unweighted(n())
  ) %>%
...
data <- data %>%
  mutate(
    wrightclass1_reduced6 = case_when(
  ...
  set_variable_labels(wrightclass1_reduced6 = "Wright Class Typology Reduced (15)")

frq(data$wrightclass1_reduced6)
...
# Tabla de frecuencias ponderadas
wright_freq <- data_svy %>%
  group_by(wrightclass1_reduced6) %>%
  summarise(
    freq = survey_mean(vartype = NULL, na.rm = TRUE),
    n_unweighted = unweighted(n())
  ) %>%
...
### wrightclass1_reduced7 (comp cl-ar)

```{r}
# Modelo para comprar con datos de Argentina 
data <- data %>%
  mutate(
    wrightclass1_reduced7 = case_when(
      as.integer(owners) == 3 & inicio_empresa == 1 ~ 1,                                                      # 1. Employers
  ...
  set_variable_labels(wrightclass1_reduced7 = "Wright Class Typology Reduced (17)")

frq(data$wrightclass1_reduced7)
...
# Tabla de frecuencias ponderadas
wright_freq <- data_svy %>%
  group_by(wrightclass1_reduced7) %>%
  summarise(
    freq = survey_mean(vartype = NULL, na.rm = TRUE),
    n_unweighted = unweighted(n())
  ) %>%

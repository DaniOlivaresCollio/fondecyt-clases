...
data <- data %>%
  mutate(
    ...
    organization_assets = case_when(
      as.integer(managerial_location_typology) %in% c(1, 2, 3, 5, 6, 7) ~ 1,
      as.integer(managerial_location_typology) == 9 ~ 2,
      as.integer(managerial_location_typology) %in% c(4, 8, 10, 11, 12) ~ 3
    ),
